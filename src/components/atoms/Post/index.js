    ...
    return (
      <div className='post'>
        ...
        <div className={detail ? 'postDetailInfo' : 'postInfo'}>
          ...
          <p className='postMinus' onClick={() => this.props.postVote(post.id, {option: 'downVote'})}><AngleDown size={25} /></p>
          <p className='scoreNumber'>{post.voteScore}</p>
          <p className='postPlus' onClick={() => this.props.postVote(post.id, {option: 'upVote'})}><AngleUp size={25} /></p>
        </div>
        <div className={detail ? 'postOps' : 'postOperations'}>
          <Link className='postEdit' to={`/edit/${post.id}`}><p className='editPost'><EditIcon /></p></Link>
          <Link className='postDelete' to='/' onClick={() => this.props.deletePost(post.id)}><DeleteIcon /></Link>
        </div>
      </div>
    )
  }
...
export default connect(mapStateToProps, { fetchComments, deletePost, postVote })(Post)
